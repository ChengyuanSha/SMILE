...
class Evolve:
    ...
    def displayHeader():
        """ Statistics header """
        print("{:^3}|{:^9}|{:^6}|{:^7}|{:^12}|{:^10}|{:^13}".format("Gen", "Best Indv", "CE", "Pop Avg", "Ran Sampling",
                                                                    "AvgProgLen", "AvgEffProgLen"))
        print(
            '-' * 3 + ' ' + '-' * 9 + ' ' + '-' * 6 + ' ' + '-' * 7 + ' ' + '-' * 12 + ' ' + '-' * 10 + ' ' + '-' * 13)

    def displayStatistics(self, g, bestIndividual, isRandomSampling, randomSz):
        """ Show generation statistics, this step is time-consuming """
        ...
        print(line_format.format(g, bestIndividual.fitness, bestIndividual.classificationError,
                                 self.p.getAverageFitness(), ranSamplingStatus, self.p.getAvgProgLen(),
                                 self.p.getAvgEffProgLen()))

    def evolveGeneration(self, pRegMut, pMicro, pMacro, pConst, pCrossover, numberOfVariable, numberOfInput,
                         numberOfOperation,
        ...
        for g in range(self.maxGeneration):
            ...
            for _ in range(len(self.p.population)):
                ...
                if np.random.random_sample() < pMacro:
                    randomInstr = Instruction(numberOfOperation, numberOfVariable, numberOfInput, numberOfConstant,
                                              pConst)
                ...
                if np.random.random_sample() < pMicro:
                    GeneticOperations.microMutation(winner1, pRegMut, pConst, numberOfVariable, numberOfInput,
                                                    numberOfOperation,
                                                    numberOfConstant)
                if np.random.random_sample() < pMicro:
                    GeneticOperations.microMutation(winner2, pRegMut, pConst, numberOfVariable, numberOfInput,
                                                    numberOfOperation,
                                                    numberOfConstant)
